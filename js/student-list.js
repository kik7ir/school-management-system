...
function viewStudentReport(admissionNumber) {
    try {
        ...
        // Redirect to the reports page with the student ID as a filter
        const url = `reports.html?studentId=${encodeURIComponent(admissionNumber)}`;
...
function formatBillingOptions(student, term = null) {
    ...
    // Get the billing options for the current term or fall back to root level billing options
    const billingOptions = student.billingOptions?.[currentTerm] || student.billingOptions || {};
    
    // If no billing options are set at all, return a message
    const hasAnyBillingOption = Object.keys(billingOptions).length > 0;
    ...
    // Process default fees with amounts if they are enabled in billingOptions
    defaultFees.forEach(({ key, format }) => {
        // Check if fee is enabled in billing options
        const isEnabled = billingOptions[key] === true;
        
        if (isEnabled) {
            // Get location for transport if it's a transport fee
            const location = key === 'transport' ? 
                (billingOptions.transportLocation || student.transportLocation) : 
                null;
    ...
    // Process custom fees from billingOptions
    Object.entries(billingOptions).forEach(([key, value]) => {
        ...
        if (typeof value === 'boolean' && value === true && !isDefaultFee && !standardProps.includes(key)) {
            ...
            if (feeEntry) {
                const [name, amount] = feeEntry;
                opts.push(`${formattedName} (${amount} KES)`);
            } else {
                // If not found in otherFees, just add the name
...
function openPayFeeModal(admissionNumber) {
    ...
    // Reset the form and set the current term
    const form = document.getElementById('payFeeForm');
    form.reset();
    
    // Set the payment term to match the current term in the student list
    const paymentTermSelect = document.getElementById('paymentTerm');
    if (paymentTermSelect) {
        // Find the option that matches the current term
        for (let i = 0; i < paymentTermSelect.options.length; i++) {
            if (paymentTermSelect.options[i].value === currentTerm) {
                paymentTermSelect.selectedIndex = i;
                break;
            }
        }
    }
    
    // Hide receipt date div by default
...
function displayStudents(term = 'Term 1') {
    ...
    students.forEach(student => {
        // Get the billing options for the current term or use root level if not set
        const termBilling = student.termBilling?.[term] || {};
        
        // Create an array to hold the billing items
        const billingItems = [];
        
        // Add school fee if enabled
        if (termBilling.schoolFee !== false) {
            billingItems.push('School Fee');
        }
        
        // Add transport if enabled
        if (termBilling.transport) {
            const location = termBilling.transportLocation || student.transportLocation || '';
            billingItems.push(`Transport${location ? ` (${location})` : ''}`);
        }
        
        // Add lunch if enabled
        if (termBilling.lunch) {
            billingItems.push('Lunch');
        }
        
        // Add other optional fees
        const optionalFees = ['reamPaper', 'activity', 'admissionFee', 'swimming', 'ujiTea', 'trip', 'snacks'];
        optionalFees.forEach(fee => {
            if (termBilling[fee]) {
                // Format the fee name (e.g., 'reamPaper' -> 'Ream Paper')
                const formattedName = fee
                    .replace(/([A-Z])/g, ' $1')
                    .replace(/^./, str => str.toUpperCase())
                    .trim();
                billingItems.push(formattedName);
            }
        });
        
        // Calculate total fee and balance for the current term
        const totalFee = calculateTotalFee(student, term);
        const feeBalance = calculateFeeBalance(student, term);
        ...
        row.innerHTML = `
            ...
            <td class="term-billing">
                ${billingItems.length > 0 ? billingItems.join(', ') : 'No billing items selected'}
            </td>
            <td class="amount">${totalFee.toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
            <td class="amount ${getBalanceClass(feeBalance)}">${feeBalance.toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
            <td>${student.transportMode || 'N/A'}</td>
            <td>${student.transportLocation || 'N/A'}</td>
            <td>${student.parentName || 'N/A'}</td>
            <td>${student.parentPhone || 'N/A'}</td>
            <td class="actions">
                <button onclick="editStudent('${student.admissionNumber}')" class="btn-edit" title="Edit">
                    <i class="fas fa-edit"></i>
                </button>
                <button onclick="openPayFeeModal('${student.admissionNumber}')" class="btn-pay" title="Pay Fee" style="background-color: #28a745; color: white; margin: 0 5px;">
                    <i class="fas fa-money-bill-wave"></i>
                </button>
                <button onclick="deleteStudent('${student.admissionNumber}')" class="btn-delete" title="Delete">
                    <i class="fas fa-trash"></i>
                </button>
            </td>
            <td>
                <button onclick="showAllPayments('${student.admissionNumber}')" class="btn-view" title="View Payment History">
...
/**
 * Get the next term in sequence (Term 1 -> Term 2 -> Term 3 -> Term 1)
 * @param {string} currentTerm - The current term ('Term 1', 'Term 2', or 'Term 3')
 * @returns {string} The next term in sequence
 */
function getNextTerm(currentTerm) {
    const terms = ['Term 1', 'Term 2', 'Term 3'];
    const currentIndex = terms.indexOf(currentTerm);
    if (currentIndex === -1) return 'Term 1'; // Default to Term 1 if invalid
    return terms[(currentIndex + 1) % terms.length]; // Wrap around to Term 1 after Term 3
}
        ...
        classStudents.forEach(student => {
            ...
            studentCard.innerHTML = `
                ...
                <div class="student-actions">
                    <button class="btn btn-view" onclick="viewStudent('${student.admissionNumber}')" title="View Student">
                        <i class="fas fa-eye"></i> View
                    </button>
                    <button class="btn btn-edit" onclick="editStudent('${student.admissionNumber}')" title="Edit Student">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn btn-pay" 
                            onclick="openPayFeeModal('${student.admissionNumber}')" 
...
// Function to update all students with current term's fee structure
function updateAllStudentsFeeStructure() {
    const students = getStudents();
    const term = document.getElementById('termSelect')?.value || 'Term 2';
    
    if (!confirm(`This will update the fee structure for ALL students for ${term}. Continue?`)) {
        return;
    }
    ...
    students.forEach(student => {
        ...
        // Initialize the term's billing if it doesn't exist
        if (!student.termBilling[term]) {
            student.termBilling[term] = {};
            updatedCount++;
        }
        
        // Copy the billing options from the previous term if they don't exist
        const previousTerm = term === 'Term 2' ? 'Term 1' : (term === 'Term 3' ? 'Term 2' : null);
        if (previousTerm && student.termBilling[previousTerm]) {
            // Only copy if the current term doesn't have these settings
            const currentTermBilling = student.termBilling[term] || {};
            const previousTermBilling = student.termBilling[previousTerm] || {};
            
            // Copy each billing option that exists in previous term but not in current term
            Object.entries(previousTermBilling).forEach(([key, value]) => {
                if (currentTermBilling[key] === undefined) {
                    currentTermBilling[key] = value;
                }
            });
            
            student.termBilling[term] = currentTermBilling;
        }
    });
    ...
    alert(`Updated fee structure for ${updatedCount} students for ${term}.`);
    
    // Refresh the display
    displayStudents(term);
}

// Add the update button to the UI
document.addEventListener('DOMContentLoaded', () => {
    const header = document.querySelector('header');
    if (header) {
        const updateButton = document.createElement('button');
        updateButton.textContent = 'Update All Students';
        updateButton.className = 'btn';
        updateButton.style.marginLeft = '10px';
        updateButton.onclick = updateAllStudentsFeeStructure;
